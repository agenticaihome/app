...
Trust and clarity are fundamental in GoP. For the current version, the following aspects are considered:
  ...
  * **Winner Validation (Post-Resolution):**
      ...
      * The winner is determined on-chain. However, to ensure fairness and verify the integrity of the `game-service`'s `blake2b256` commitment engine, a **Judge System** is in place. The creator nominates a set of judges (`participatingJudges`) upon game creation. 
      * **Judge Commitments (The R9 Requirement):** When judges accept their nomination, they must play the game and provide their own reference participation data (their `commitmentC`, `solverId`, `score`, `hashLogs`, and `ergoTree`). This data is stored in the `R9` register of their nomination acceptance transaction. During the on-chain resolution phase, the `game_active.es` contract mathematically verifies that *every* participating judge's `commitmentC` can be correctly reconstructed using their provided data and the creator-revealed secret `S`. If even one judge's commitment is invalid, the resolution transaction fails. This ensures the game-service correctly generates commitments and that judges have actively participated and verified the gameplay.
      * During the resolution phase, if judges detect fraud or inconsistencies in the winner's logs (e.g., by replaying the game and finding discrepancies), they can vote to invalidate the participation. **Crucially, this process judges the integrity of the Creator's Game Service, not the player.** Since the `commitmentC` and logs are generated by the service provided by the creator, an invalidation implies that the Game Service failed to generate a valid proof. 
      * **This action does NOT cancel the game.** Instead, it penalizes the creator by **transferring their entire commission to the Judges' pool**. This creates a robust incentive structure: the Creator is responsible for nominating trustworthy judges, and the Judges are incentivized to rigorously verify both the Game Service's mathematical integrity (via R9 commitments) and its gameplay fairness, as they directly benefit from penalizing a faulty or fraudulent service. The resolution deadline is also extended to allow the protocol to continue and find a valid winner. A majority vote is required to invalidate a candidate.
