...
describe.each(baseModes)("Game Resolution Invalidation by Judges (Refactored) - (%s)", (mode) => {
    ...
    it("should successfully invalidate the current winner with a majority of judge votes (2 out of 3)", () => {
        ...
        judge1.addBalance({ nanoergs: 1_000_000_000n });
        ...
        // Must contain Judges in R4 and Params in R5
        const params = [
            1n, 20n, 700_000n, 2_000_000_000n, 1_000_000n, 1n, 10n
        ];
        ...
        configBox = {
            ...
            additionalRegisters: {
                R4: SColl(SColl(SByte), judges).toHex(), // Judges List
        ...
        gameResolutionContract.addUTxOs({
            ...
            additionalRegisters: {
                ...
                R6: SPair(SColl(SByte, secret), SColl(SByte, invalidatedCommitment)).toHex(),
        ...
        // --- Create Participation Box (Incriminated) ---
        participationContract.addUTxOs({
            ...
            additionalRegisters: {
                ...
                R5: SColl(SByte, hexToBytes(invalidatedCommitment)!).toHex(),
        ...
        reputationProofContract.addUTxOs({
            ...
            additionalRegisters: {
                ...
                R8: SBool(false).toHex(), // INVALID
                R9: SColl(SByte, []).toHex(),
            }
        });
        // Judge 2 Vote (Invalid)
        reputationProofContract.addUTxOs({
            creationHeight: mockChain.height - 10,
            ergoTree: reputationProofErgoTree.toHex(),
            value: 1_000_000n,
        ...
        // New State:
        // Winner (R6._2) = Empty
        // Params (R7) = Extended Deadline (resolutionDeadline + JUDGE_PERIOD)
        // Wait, R7 is now [deadline, comms]. So index 0 changes.
            ...
            .to([
                new OutputBuilder(newFunds, gameResolutionErgoTree)
                    ...
                    .setAdditionalRegisters({
                        ...
                        R6: SPair(SColl(SByte, secret), SColl(SByte, [])).toHex(), // Winner cleared
                        // R7 updated with new deadline
                        R7: SColl(SLong, [extendedDeadline, 1n, 10n]).toHex(),
                        R8: gameResolutionBox.additionalRegisters.R8,
                        R9: gameResolutionBox.additionalRegisters.R9
                    })
            ])
            // MUST include Config Box for Judge Verification (R4 of Config)
            .withDataFrom([
